...
# CSV file selection (structured)
version = st.sidebar.selectbox("Version", ["v1"], index=0)
input_mode = st.sidebar.selectbox("Input Mode", ["image", "stroke"], index=0)
step = st.sidebar.selectbox("Step", [100, 200], index=2)

# Construct CSV file name based on selections
csv_file = f"{input_mode}_{version}_step{step}.csv"
...
selected_row = st.selectbox("Select an incorrect prediction to inspect", incorrect_df.index, format_func=lambda idx: f"{incorrect_df.at[idx, 'category']} (Predicted: {incorrect_df.at[idx, 'prediction']})")
...
# If stroke data available, render image
